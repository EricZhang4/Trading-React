...
export const fetchMarketChart =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      dispatch({ type: FETCH_MARKET_CHART_SUCCESS, payload: user.jwt });
    } catch (error) {
      console.log("error", error);
...
export const fetchCoinDetails =
  ...
  async (dispatch) => {
    ...
    try {
      const response = await api.get(`/coin/details/${coinId}`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
  ...
  export const searchCoin = (keyword) =>async (dispatch) => {
    ...
    try {
      const response = await api.get(`/coin/search?q=${keyword}`);
